...
import { useGetInstructorCourseTestsQuery } from "../../../redux/slices/instructor/instructorApiSlice";
...
export const InstructorCourseTests = () => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetInstructorCourseTestsQuery({ instructorId, courseId });
  ...
  // Load existing tests (mock API or real API)
  useEffect(() => {
    if (tests) {
      setCurrentTest({
        title: "",
  ...
  const handleSaveTest = async () => {
    ...
    try {
      ...
      if (currentTest.id) {
        // Cập nhật bài kiểm tra nếu có ID
        await manageTests({
          courseId,
